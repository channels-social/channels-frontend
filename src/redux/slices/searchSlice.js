...
export const fetchCurationSearch = createAsyncThunk('globalSearchItems/curations', async (query, { rejectWithValue }) => {
    try {
      const response = await postRequestUnAuthenticated('/global/search/curations', {query });
      if (response.success) {
...
export const fetchProfileSearch = createAsyncThunk('globalSearchItems/profiles', async (query, { rejectWithValue }) => {
    try {
      const response = await postRequestUnAuthenticated('/global/search/profiles', {query });
      if (response.success) {
