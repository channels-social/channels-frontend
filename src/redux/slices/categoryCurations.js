...
export const fetchCurationsfromCategory = createAsyncThunk('categories/fetchCurations', async (category, { rejectWithValue }) => {
  try {
    const response = await postRequestUnAuthenticated(`/fetch/category/curations`, { category: category });
...
export const fetchChips = createAsyncThunk('categories/fetchChips', async (curId, { rejectWithValue }) => {
  try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(saveCuration.fulfilled, (state, action) => {
        ...
        const index = state.curations.findIndex(curation => curation._id === updatedCuration._id);
        if (index !== -1) {
...
export const { setSelectedCuration, setError,clearCurationChips } = categoryCurationSlice.actions;
