...
import { createChip, updateChip,upvoteChip,saveChip,shareChips } from './profileItemsSlice';
import { deleteChip } from './deleteChipSlice';
import {createChipComment, createChipCommentReply } from './commentChipSlice';

export const fetchCuration = createAsyncThunk('curation/fetchCuration', async (curId, { rejectWithValue }) => {
  try {
    const response = await postRequestUnAuthenticated(`/fetch/curation/from/curationId`, { curation_id:curId });
    if (response.success) {
...
export const fetchChips = createAsyncThunk('curation/fetchChips', async (curId, { rejectWithValue }) => {
  try {
...
export const saveCuration = createAsyncThunk('curation/saveCuration', async (curationId, { getState, rejectWithValue }) => {
    try {
      const response = await postRequestAuthenticated('/toggle/save/curation', { curation_id: curationId });
...
export const shareCuration = createAsyncThunk('curation/shareCuration', async (curationId, {rejectWithValue }) => {
    try {
      const response = await postRequestUnAuthenticated('/curation/shared_by', { curation_id: curationId });
      if (response.success) {
        return response.curation;
      } else {
        return rejectWithValue(response.message);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateChip.fulfilled, (state, action) => {
        if (state.curation._id === action.payload.curation) {
          const index = state.chips.findIndex(chip => chip._id === action.payload._id);
          if (index !== -1) {
    ...
    .addCase(createChipCommentReply.fulfilled, (state, action) => {
      ...
      .addCase(upvoteChip.fulfilled, (state, action) => {
        const updatedChip = action.payload;
        const index = state.chips.findIndex(item => item._id === updatedChip._id);  
        if (index !== -1) {
...
export const { setCurationField,updateCurationPage,updateChipPage,clearItems } = curationPageSlice.actions;
