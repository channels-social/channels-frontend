...
const Unsplash = () => {
  ...
  const fetchImages = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${hostUrl}/api/photos/search?keyword=${encodeURIComponent(searchQuery)}&limit=8`);
      const data = response.data;
      if (data.success) {
        setImages(data.data.slice(0, 8).map(image => ({
          ...image,
  ...
  return (
    <Dialog.Root open={isOpen}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black bg-opacity-70 z-50" onClick={handleOverlayClick} />
        <div className="fixed z-50 inset-0 flex items-center justify-center">
          ...
          <Dialog.Title/>
            ...
            <div className="flex justify-between items-center mb-4 pr-3">
              <h2 className="text-white text-lg font-normal fonr-inter">Unsplash</h2>
              <img src={Close} alt="Close" className="w-6 h-6 cursor-pointer" onClick={handleClose} />
            </div>
            <div className="flex flex-row items-center mb-4">
              <div className="relative w-full">
                <FontAwesomeIcon icon={faSearch} className="absolute top-1/2 ml-3 transform -translate-y-1/2 text-white w-4 h-4" />
                <input
                  type="text"
                  placeholder="Search"
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-2 gap-4 mt-4 mr-3">
                  {images.map((image, index) => (
                    ...
                    >
                      <img src={image.url} alt={image.name} className="w-full h-44 object-cover rounded-lg" />
                      ...
                      {selectedImage === image.url && (
                        <div className="absolute top-0 right-0 rounded-full border items-center border-white bg-selectedColor w-5 h-5"> 
                          <FontAwesomeIcon icon={faCheck} className="absolute top-0 right-0 w-4 h-4" style={{ color: 'white' }} />
                        </div>
                      )}
                    </div>
                  ))}
