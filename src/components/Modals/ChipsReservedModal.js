...
import { setLocationField, setDateField, setDocumentField, setChipField,setImageField,setAllImagesField } from '../../redux/slices/chipSlice';
import DocumentPreview from './../chips/widgets/DocumentPreview';
import ReservedImages from './widgets/ReservedImages';


const ChipsReservedModal = ({onBack,handleClose,postButton,handleSave,Chipstatus}) => {
    ...
    const handleLocationExclusiveChange = (e) => {
      ...
      const handleLinkExclusiveChange = (e) => {
        dispatch(setChipField({ field: 'link_exclusive', value: e.target.checked }));
      };
      const handleTextExclusiveChange = (e) => {
        dispatch(setChipField({ field: 'text_exclusive', value: e.target.checked }));
      };
      const handleImageExclusiveChange = (index) => {
        dispatch(setImageField({ field: index, value: !chipData.image_urls[index].exclusive }));
      };
    ...
    <div className="flex flex-col h-full">
     ...
     {chipData.text &&  <div className="flex items-center space-x-4 mr-2">
      ...
      { chipData.document && chipData.document.url && <div className="flex items-center space-x-4 mr-4">
        ...
        <button className={`w-full  py-2.5 mt-3 rounded-full ${postButton} font-normal`} onClick={handleSave}>{Chipstatus==="loading"?"Please wait...":"Post with reserved notes"}</button>
