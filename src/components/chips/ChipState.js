...
useEffect(() => {
  ...
  const handleSubscribe= async()=> {
    if(isLoggedIn){
      try {
        const response = await postRequestAuthenticated('/toggle/subscription',{receiverId: profileData._id});
        if (response.success) {
  ...
  return (
    <div className="w-[98%] flex flex-col sm:flex-row bg-primaryBackground min-h-screen ">
        ...
        <div className="relative w-full h-full pr-2">
          <img src={chipBackground} alt="Chip state" className=" hidden sm:flex absolute lg:top-0 md:-top-1/4 sm:-top-1/3 left-0 w-full h-full object-cover" /> {/* Cover entire container */}
          ...
          { chipItemData.chip.curation &&  <div className="flex flex-row items-center mb-3 w-max cursor-pointer"onClick={()=>navigate(`/curation/${chipItemData.chip.curation._id}`)}>
            <img src={chipItemData.chip.curation.image} alt="curation-image" className="w-14 h-10 rounded-md mr-4"/>
          <p className="text-white text-base  font-normal">{chipItemData.chip.curation.name}</p>
            </div>}
            {chipItemData.chip && chipItemData.chip.user && (owner?
            <ProfileChips item={chipItemData.chip}/>: <Chips item={chipItemData.chip} />)}
          </div>
        </div>
      </div>
            <div className="flex flex-col rounded-md sm:mt-0 mt-6 sm:w-1/2 w-[98%] bg-chipBackground p-6">
                    <div className="flex flex-row justify-between items-center">
                        <h1 className="lg:text-3xl md:text-2xl xs:text-xl text-md text-white font-normal font-familjen-grotesk">More from the author</h1>
                        <button className="bg-primary sm:px-4 px-1 py-1.5 rounded-3xl text-buttonText  font-inter text-xs"
                    ...
                    <div className="mt-4 bg-profileBackground flex py-4 px-5 flex-col items-center justify-center rounded-lg w-full">
                            ...
                            <div className={`mt-1 text-center`}>
                                <h1 className="text-xl text-white font-normal font-familjen-grotesk">{profileData.name}</h1>
                                ...
                                <div className={`flex space-x-4 justify-center `}>
                                  {profileData.customText && profileData.customUrl &&  <a href={profileData.customUrl} className="px-5 mt-4  py-2 bg-primary text-buttonText text-sm rounded-lg">{profileData.customText}</a>}
