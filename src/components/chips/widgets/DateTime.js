...
const DateTimeCard = ({ item }) => {
  ...
  return (
    <div className="w-[95%] dark:bg-secondaryBackground-dark bg-opacity-20 rounded-tr-xl rounded-br-xl overflow-hidden mr-4 pr-1 flex items-center">
      <div className="relative flex items-start space-x-4 py-0.5">
        <div className="absolute left-0 top-0 bottom-0 w-1.5 rounded-lg dark:bg-sidebarColor-dark"></div>
        <div
          className={`flex flex-col pt-1 pb-1 ml-4 mr-2 ${
            item.end_time ? "w-2/5" : "w-1/3"
          }`}
        >
          <div className="dark:text-secondaryText-dark font-inter font-medium text-sm">
            {date}
          </div>
          <div className="dark:text-secondaryText-dark text-[10px] font-inter mt-1 font-light">
            {time}
          </div>
        </div>
        <div className="mt-1 ml-4">
          <div className="dark:text-secondaryText-dark text-sm font-inter font-normal">
            {item.event}
          </div>
        </div>
      </div>
